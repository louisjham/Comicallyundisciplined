...
const ComicPanel: React.FC<ComicPanelProps> = ({ panel, index }) => {
  return (
    <div className="relative h-full flex flex-col pt-6 group">
      {/* Panel Label - Moved out of the way, styled as an evidence tab */}
      <div className="absolute top-0 right-6 z-30 pointer-events-none">
        <div className="bg-red-600 text-white px-3 py-1 text-[10px] font-bold border-2 border-black uppercase tracking-widest shadow-[3px_3px_0_0_rgba(0,0,0,1)] transform rotate-1 group-hover:rotate-0 transition-transform">
          REPORT ITEM #{index + 1}
        </div>
      </div>

      <div className="relative overflow-hidden bg-white comic-border flex-1 flex flex-col">
        {/* Narration box */}
